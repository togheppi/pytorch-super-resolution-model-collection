...
class Net(torch.nn.Module):
    def __init__(self, num_channels, base_filter, num_inferences):
        ...
        for _ in range(num_inferences):
            self.conv_blocks.append(ConvBlock(base_filter, base_filter, 3, 1, 1, norm=None))
        self.inference_layer = nn.Sequential(*self.conv_blocks)

        init_w = torch.ones(num_inferences) / num_inferences
        self.w = Variable(init_w, requires_grad=True).cuda()
        w_sum = torch.sum(self.w)
        for i in range(num_inferences):
            self.conv_blocks.append(self.w[i] * ConvBlock(base_filter, base_filter, 3, 1, 1, norm=None) / w_sum)
        self.inference_layer = nn.Sequential(*self.conv_blocks)
